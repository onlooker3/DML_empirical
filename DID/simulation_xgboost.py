...
def did_cs(n_obs, index):
    ...
    if n_obs == 500:
        ml_g_xgb = XGBRegressor(n_estimators=18)
        ml_m_xgb = XGBClassifier(n_estimators=18)
    elif n_obs == 1000:
        ml_g_xgb = XGBRegressor(n_estimators=18)
        ml_m_xgb = XGBClassifier(n_estimators=18)
    elif n_obs == 5000:
    ...
    dml_did_xgb = dml.DoubleMLDIDCS(dml_data, ml_g=ml_g_xgb, ml_m=ml_m_xgb, score='observational', in_sample_normalization=True, n_folds=5)
    dml_did_xgb.fit()

    return dml_did_xgb.coef[0]
...
for num_obs in num_obs_seq:
    ...
    output_xgb = np.array(output)

    print(num_obs)
    print(np.mean(output_xgb))
    print(np.std(output_xgb))
