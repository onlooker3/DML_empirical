...
def plr_rf(max_depth, min_split, index):
    warnings.filterwarnings("ignore")
    ...
    ml_l = DecisionTreeRegressor(max_depth=max_depth, min_samples_split=min_split)
    ml_m = DecisionTreeClassifier(max_depth=max_depth, min_samples_split=min_split)
...
result_pred = np.zeros(shape=len(Max_depth) * len(Min_split))
result_est= np.zeros(shape=len(Max_depth) * len(Min_split))

n = 0
for n1 in range(len(Max_depth)):
    for n2 in range(len(Min_split)):

        output = Parallel(n_jobs=14)(delayed(plr_rf)(Max_depth[n1], Min_split[n2], index) for index in range(Nrep))
...
np.savetxt('result_tree.txt', np.array([result_pred, result_est]), fmt ='%f', delimiter='\t')