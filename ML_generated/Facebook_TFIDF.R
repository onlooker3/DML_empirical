...
text = rbind(as.matrix(text_label), as.matrix(text_unlabel))
...
for(k in 1:K)
{
  ...
  text_other = text[folding != k, ]
  sentiment_other_obs = sentiment[folding != k & label == 1, ]
  label_other = label[folding != k]
  
  ntrees = 100
  lambda = mean(label[folding!=k])
  lambdaList = c(lambdaList, list(lambda))
  ...
  mu2 = mu1
  mu1List = c(mu1List, list(mu1))
  mu2List = c(mu2List, list(mu2))
